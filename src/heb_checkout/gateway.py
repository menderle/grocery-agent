...
def build_gateway() -> FastMCP:
    ...
    gateway = FastMCP(
        ...
        instructions=(
            ...
            "  3. After answers, search each item, add to cart, then call cart_get to show "
            "the itemized cart + subtotal, and get_slots for pickup/delivery times.\n\n"
            "SHOWING THE ORDER before placing: use cart_get — an INSTANT API read of the cart "
            "(items + subtotal). Do NOT use preview_order for this: it drives a slow (~30s), "
            "flaky browser checkout walk and routinely times out. The cart_get subtotal is the "
            "number to show the user — groceries are largely tax-exempt and H-E-B pickup is "
            "free, so subtotal effectively IS the total; place_order then confirms the exact "
            "on-screen total at checkout and aborts if it comes out >10% higher. Reserve "
            "preview_order only for when the user explicitly asks for a full fees/tax review.\n\n"
